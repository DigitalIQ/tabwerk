...
export function timeWindow(query, now = Date.now()) {
  ...
  if (/\bheute\b|\btoday\b/.test(q)) return { from: today, to: now, label: t('tw_today') };
  if (/\bgestern\b|\byesterday\b/.test(q)) return { from: today - DAY, to: today, label: t('tw_yesterday') };
  if (/vorgestern/.test(q)) return { from: today - 2 * DAY, to: today - DAY, label: t('tw_dayBeforeYesterday') };
  if (/letzte[nr]? woche|last week|vorige[nr]? woche/.test(q)) {
    const dow = (new Date(today).getDay() + 6) % 7;
    const thisMonday = today - dow * DAY;
    return { from: thisMonday - 7 * DAY, to: thisMonday, label: t('tw_lastWeek') };
  }
  if (/diese[nr]? woche|this week/.test(q)) {
    const dow = (new Date(today).getDay() + 6) % 7;
    return { from: today - dow * DAY, to: now, label: t('tw_thisWeek') };
  }
  if (/letzte[nr]? monat|last month|vorige[nr]? monat/.test(q)) {
    ...
    return { from, to, label: t('tw_lastMonth') };
  }
  const m = /vor (\d+) tagen|(\d+) days ago/.exec(q);
  if (m) {
    const n = Number(m[1] || m[2]);
    return { from: today - n * DAY - DAY, to: today - n * DAY + DAY, label: tp('tw_daysAgo', n) };
  }
  return { from: now - 90 * DAY, to: now, label: t('tw_last90Days') };
}
